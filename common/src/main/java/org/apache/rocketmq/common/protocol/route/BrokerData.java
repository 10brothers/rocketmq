...
/** broker的信息表示，所属集群名，broker名，以及这个broker名中的brokerId和地址映射。
 * 一个cluster可以有多个broker，根据brokerName来区分，相同的brokerName组成主从高可用，broker的高可用集群使用brokerId来区分主从。
 * 一个Broker可以主从部署，id=0的为master，其他为follower。主从broker根据brokerId来识别*/
